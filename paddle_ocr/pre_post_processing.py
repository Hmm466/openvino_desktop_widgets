...
postprocess_params = {
    "name": "CTCLabelDecode",
    "character_type": "ch",
    "character_dict_path": f"_internal/openvino_desktop_widgets/paddle_ocr/ppocr_keys_v1.txt",
    "use_space_char": True,
}
...
def draw_ocr_box_txt(image, boxes, txts, scores=None, drop_score=0.5):
    np.random.seed(0)
    # 创建一个和image大小相同的全黑色遮罩，用于绘制半透明的矩形
    overlay = image.copy()
    alpha = 0.5
    for idx, (box, txt) in enumerate(zip(boxes, txts)):
        ...
        # 绘制半透明矩形
        cv2.fillPoly(overlay, [np.array(box, np.int32)], color)
    # 应用加权混合，得到半透明效果
...
class PaddleOCR:
    ...
    def process_frame(self, frame, processing_times,det_compiled_model,det_output_layer,rec_compiled_model,rec_output_layer):
        ...
        for beg_img_no in range(0, img_num, batch_num):
            ...
            norm_img_batch = self.batch_text_box(img_crop_list, img_num, indices, beg_img_no, batch_num)
        ...
        # print(txts)
        # print(boxes)
        draw_img = draw_ocr_box_txt(frame, boxes, txts, scores, drop_score=0.5)
